...
try:
    ...
    try:
        ...
        sys.exit("[!] wrong installation detected (missing modules). Visit 'https://github.com/yourusername/nostradamus/#installation' for further details")
...
def main():
    ...
    try:
        ...
        # Show predictor efficiency summary if it was used
        if kb.get("predictor") and (kb.predictor.stats_hits or kb.predictor.stats_misses):
            report = kb.predictor.get_efficiency_report()
            if report:
                for line in report:
                    logger.info(line)
